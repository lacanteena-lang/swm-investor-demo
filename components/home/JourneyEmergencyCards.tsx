...
export default function SafetyProfile() {
  ...
  // Restore saved safety information when Profile is opened again.
  useEffect(() => {
    const savedBloodGroup = window.localStorage.getItem("swm-blood-group");
    const savedMedicalInfo = window.localStorage.getItem("swm-medical-info");
    const savedAddress = window.localStorage.getItem("swm-home-address");

    if (savedBloodGroup) setBloodGroup(savedBloodGroup);
    ...
    if (savedAddress) {
      try {
        ...
        // Ignore an invalid old saved value and keep the form empty.
      }
    }
  }, []);
  ...
  const saveMedicalInfo = () => {
    ...
    window.localStorage.setItem("swm-medical-info", medicalInfo.trim());
    setSelectedItem(null);
  };
  ...
  const saveAddress = () => {
    ...
    window.localStorage.setItem("swm-home-address", JSON.stringify(addressForm));
    setSelectedItem(null);
  };
