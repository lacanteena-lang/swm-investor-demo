...
export default function SafetyProfile() {
  ...
  // Load the user's safety profile from Supabase when the screen opens.
  useEffect(() => {
    let cancelled = false;
    ...
    const loadSafetyProfile = async () => {
      try {
        ...
        // Anonymous auth is enabled for this project. If there is no
        // existing session, create one so the safety profile can be saved.
        if (!user) {
        ...
        // Ensure the public profile row exists before inserting the
        // safety profile because safety_profiles.user_id references it.
        ...
        const [{ data: safetyProfile, error: safetyError }, { data: emergencyContacts, error: contactsError }] =
          await Promise.all([
            supabase
              .from("safety_profiles")
              .select("blood_group, medical_info, address_line, city, state, pin_code")
              .eq("user_id", user.id)
              .maybeSingle(),
            supabase
        ...
        if (safetyProfile) {
          setBloodGroup(safetyProfile.blood_group ?? "");
          setMedicalInfo(safetyProfile.medical_info ?? "");
          setAddress({
            addressLine: safetyProfile.address_line ?? "",
            city: safetyProfile.city ?? "",
            state: safetyProfile.state ?? "",
            pinCode: safetyProfile.pin_code ?? "",
          });
          setAddressForm({
            addressLine: safetyProfile.address_line ?? "",
            city: safetyProfile.city ?? "",
        ...
        const loadedContacts = (emergencyContacts ?? [])
          .slice(0, 5)
          .map((contact) => ({
            name: contact.name ?? "",
        ...
        setContacts([
          ...loadedContacts,
          ...Array.from(
            { length: Math.max(0, 5 - loadedContacts.length) },
            () => emptyContact()
          ),
        ]);
      } catch (error) {
        console.error("Unable to load safety profile:", JSON.stringify(error, null, 2));
      }
    };
  ...
  const saveSafetyProfile = async (values: {
    bloodGroup: string;
    medicalInfo: string;
    address: Address;
  }) => {
    ...
    const { error } = await supabase.from("safety_profiles").upsert(
      {
        user_id: user.id,
        blood_group: values.bloodGroup || null,
        medical_info: values.medicalInfo || null,
        address_line: values.address.addressLine || null,
        city: values.address.city || null,
        state: values.address.state || null,
        pin_code: values.address.pinCode || null,
      },
      { onConflict: "user_id" }
    );
  ...
  const saveEmergencyContacts = async (nextContacts: EmergencyContact[]) => {
    const user = await getCurrentUser();
    if (!user) throw new Error("No authenticated user.");
    ...
    const validContacts = nextContacts.filter(
      (contact) =>
        contact.name.trim() &&
  ...
  const selectBloodGroup = async (group: string) => {
    try {
      ...
      console.error("Unable to save blood group:", JSON.stringify(error, null, 2));
    }
  };
  ...
  const saveMedicalInfo = async () => {
    ...
    try {
      ...
      console.error("Unable to save medical information:", error);
    }
  };
  ...
  const saveAddress = async () => {
    ...
    try {
      ...
      console.error("Unable to save home address:", error);
    }
  };
